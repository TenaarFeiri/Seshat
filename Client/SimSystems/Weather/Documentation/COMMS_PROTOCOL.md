...
Every message is a JSON object with six keys. The `q` field is a transaction sequence ID used to correlate a request, its routed response, and diagnostic acknowledgements:

```json
{
  "o": [1, 2],
  "q": "GRID-1730000000-42",
  "f": "object_uuid",
  "s": "script_id",
  "t": [1, 2],
  "p": [payload1, payload2]
...
- **`q`** — sequence. Transaction ID generated by the originating script. Routed messages and ACKs preserve this value. It is a string and is unique for the originator's active message stream.
...
The `q` sequence is not a fragment or reassembly ID. It correlates the logical transaction across separate request, response, and ACK messages.

## ACKs

`ACK` is operation 15. ACK payloads use the schema:

```lua
p = {{sequence, stage, detail}}
```

The envelope `q` and payload `sequence` both identify the transaction. ACKs are not ACKed, which prevents acknowledgement loops. ACKs are diagnostic and do not retry or queue work.

The weather path reports these stages:

| Stage | Sender | Meaning |
|---|---|---|
| `MAIN_RX` | Main → Grid | Main parsed and validated a grid message |
| `PROC1_RESP` | Main → Grid | Main received a STATE_RESP from Proc1 |
| `GRID_RX` | Grid → Main | Grid handled a non-response Main message |
| `GRID_STATE_RX` | Grid → Main | Grid handled a STATE_RESP and completed its response handler |

For a normal poll, the sequence is:

1. Grid sends `STATE_POLL` with a new `q`.
2. Main validates it and sends `MAIN_RX` to the grid.
3. Main forwards `STATE_POLL` to Proc1 with the same `q`.
4. Proc1 sends `STATE_RESP` to Main with the same `q`.
5. Main sends `PROC1_RESP` to the grid and forwards `STATE_RESP` with the same `q`.
6. Grid sends `GRID_STATE_RX` to Main after handling the response.

This distinguishes a dead or disconnected grid from a dead Main or Proc1. Main also warns when a registered grid continues sending polls but stops returning `GRID_STATE_RX` acknowledgements.

Messages without `q` remain accepted for rolling compatibility with older deployed scripts, but they cannot participate in transaction diagnostics.
...
local op = {
    ...
    ADMIN_HISTORY  = 14,  -- Main → Grid: dump transition history
...
Diagnostic acknowledgement for a sequenced message. The payload is `[sequence, stage, detail]`; the same sequence is also carried in the envelope's `q` field. ACK messages are not ACKed.
...
local msg = {
    o = {3},                              -- STATE_POLL
    q = "GRID-1730000000-42",             -- transaction sequence
...
local msg = {
    o = {7},                              -- TARGET_PUSH
    q = "GRID-1730000000-43",             -- transaction sequence
    f = ll.GetKey(),                      -- object UUID (raw key, no concatenation)
    s = SCRIPT_ID,                        -- "GRID"
    t = {1},                              -- Main
...
local msg = {
    o = {5},                              -- META_REQ
    q = "PROC1-1730000000-7",             -- transaction sequence
    f = ll.GetKey(),                      -- object UUID (raw key, no concatenation)
    s = SCRIPT_ID,                        -- "PROC1"
...
| 15 | `ACK` | Main ↔ Grid | Diagnostic acknowledgement for a sequenced transaction |
