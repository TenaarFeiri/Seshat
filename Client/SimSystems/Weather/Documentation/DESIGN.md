...
- **Processor object** (single prim): Contains four scripts sharing one 128 KB LSD. Inter-script communication uses `llMessageLinked(LINK_THIS, ...)` (link messages work within a single prim, reaching all scripts in it). LSD write events also fire for all scripts in the prim, providing an additional coordination signal. The single-prim structure simplifies rez and distribution — one object, one LSD, no linkset traversal needed.
  ...
  - **Proc2 (auxiliary)**: Handles micro compute — temperature, humidity, wind, and other computed values, evolved toward grid-provided targets. Same read-all, write-own-keys discipline as Proc1, writing to `grid_uuid:micro:*`. Works independently of Proc1; both read from the shared namespace and global drivers, feeding off each other's results. Stale reads are acceptable because weather is slow. Wind is now read from `drivers:wind` (published by Proc3) instead of relaxing toward notecard wind targets — Proc2 consumes the driver's computed wind and blends it with its own micro evolution. Proc2 writes `drivers:conditions` (temp, humidity) for Proc3's pressure-wind coupling. During state transitions, Proc2 interpolates all numeric targets over a configurable ramp period (two-phase transitions — see "Two-phase transitions" below).
  - **Proc3 (env)**: Handles global environmental drivers — things computed once per cycle and exposed to all grids. Owns the `drivers:pressure` and `drivers:wind` write surfaces (Proc2 owns `drivers:conditions`). Runs on its own cycle cadence, which may differ from Proc1/Proc2 (e.g., flood state is a calendar lookup that changes slowly; pressure and wind trends update more frequently). Proc1 and Proc2 read `drivers:*` keys when processing each grid. Reads the `grids` registry and `<grid_uuid>:targets` to configure its wind driver (base speed/direction and variability per grid) and to detect state modifier changes that require re-reading wind base values. Reads `drivers:conditions` (temp, humidity written by Proc2) for pressure-wind coupling. First implementation: Nile flood state, background atmospheric pressure variation (with thermal and moisture coupling from conditions data), and wind speed/direction published via `drivers:wind` (calm/gust oscillation, diurnal sea/land breeze, pressure-gradient wind). The pressure driver provides an independent external signal that pushes computed pressure values away from grid-specified targets, giving grids something external to evaluate when deciding state transitions. Future: extreme weather emergence (Phase 6, not yet implemented), seasonal event flags, admin-forced conditions.
...
- **Background atmospheric pressure** (computed by Proc3): an independent pressure signal with thermal and moisture coupling from conditions data. Proc2 mixes this into its pressure evolution.
- **Wind** (computed by Proc3): wind speed and direction published via `drivers:wind`, combining calm/gust oscillation, diurnal sea/land breeze (using the grid's `sea_direction`), and pressure-gradient wind. Proc2 consumes this instead of relaxing toward notecard wind targets.
...
| Proc3 (drivers) | 5 seconds | More aggressive — pressure and wind drivers need fine-grained updates |
...
{season: Akhet}
  # Season-level wind defaults (override grid-level wind_base_*).
  wind_base_speed = 5
  wind_base_dir = NW
...
| `wind_speed_mod` | number (delta) | Wind speed modifier (kph) applied to the season-level `wind_base_speed`. Replaces the old absolute `wind_speed`. Use `0` for no change. |
| `wind_dir_mod` | cardinal or degrees | Wind direction modifier applied to the season-level `wind_base_dir`. Replaces the old absolute `wind_dir`. Use `0` for no change. |
| `wind_variability_mod` | number (delta, 0-1) | Wind variability modifier applied to the season-level `wind_variability`. Replaces the old absolute `wind_variability`. Use `0` for no change. |
...
| Field | Type | Purpose |
|---|---|---|
| `wind_base_speed` | number | Base wind speed (kph) for this season. Overrides the grid-level default. State-level `wind_speed_mod` is added to this. |
| `wind_base_dir` | cardinal or degrees | Base prevailing wind direction for this season. Overrides the grid-level default. State-level `wind_dir_mod` is applied to this. |
| `wind_variability` | 0-1 | Base wind variability for this season. Overrides the grid-level default. State-level `wind_variability_mod` is added to this. |
...
When a grid decides a state transition, it uses a **two-phase transition** mechanism to avoid abrupt visual and parameter snaps:

1. **Phase 1 — TARGET_PUSH (immediate):** The grid sends TARGET_PUSH with the new state's parameters right away. Proc2 begins interpolating all numeric targets over a ramp period toward the new values, and Proc3 begins ramping its wind driver toward the new base values. This produces a smooth parameter drift (wind, temperature, humidity, pressure) rather than an instant jump.
2. **Phase 2 — EEP/particle switch (delayed):** The grid delays switching the EEP environment preset and particle effects for ~5 minutes (configurable via `wind_ramp_seconds` in the targets payload). This gives the computed values time to settle into the new regime before the visuals change, so the scene doesn't visually snap while the numbers are still mid-transition.

**Bootstrap and force transitions skip the ramp.** When a grid jumps directly to a state on first poll (bootstrap adjustment) or is forced via the `force` admin command, it sends TARGET_PUSH and applies the EEP/particle switch immediately — no ramp, no delay. This is intentional: bootstrap and force are explicit overrides where an instant transition is desired.
...
1. Processor object boots. Main starts listening for registrations. Proc1 and Proc2 begin idle — they read the `grids` registry from LSD and round-robin through whatever namespaces exist. Proc3 begins computing global drivers immediately (flood state, background pressure, wind) on its own 5-second cadence; wind configuration for individual grids begins once grids register and appear in the `grids` registry.
...
4. Proc1 and Proc2 **populate** the namespace keys created by Main. They do not create keys themselves — Main owns the namespace lifecycle. Proc1 and Proc2 request metadata from the grid via Main if it's not already present in the namespace. Proc3 reads the `grids` registry and `<grid_uuid>:targets` to configure its wind driver, but does not populate per-grid namespace keys.
5. Processors begin cycling. Proc1 and Proc2 evolve per-grid values toward the grid-provided targets on their 15-second cadence, one grid per cycle, queuing the next. Proc3 computes global drivers to `drivers:*` (pressure, wind, conditions) on its 5-second cadence, reading `<grid_uuid>:targets` to configure wind per grid. Proc1 and Proc2 read `drivers:*` when processing each grid.
...
Partial boot is functional, not broken. Late-registering grids appear in the `grids` registry and are picked up on Proc1/Proc2's next round-robin pass and on Proc3's next cycle (for wind configuration). Processors can begin cycling on whatever grids are in the registry immediately — it does not have to be all at once. Proc3 computes pressure and flood drivers regardless of how many grids are registered; wind configuration for a grid begins once that grid appears in the registry.
...
3. **Main signals procs** to resume. Proc1 and Proc2 read the `grids` registry and resume their round-robin. Proc3 reads the `grids` registry and `<grid_uuid>:targets` to reconfigure its wind driver, and resumes computing drivers. No proc needs to re-boot or re-initialize — they just start cycling again.
...
- **Proc3 resets**: `drivers:*` keys are owned by Main (scaffolded at boot). Proc3 re-reads its internal state from `drivers:*` if Main stored it there, or re-initializes from scratch (flood state from date, pressure driver phase = 0, wind driver reconfigured from `<grid_uuid>:targets`). The pressure driver's sinusoidal phase restarts, which may cause a brief discontinuity in the pressure signal. This is acceptable — the grid's 5-minute trend window will smooth over it within a few minutes.

## Failure handling

- **Grid prim deleted**: Main detects no response within a timeout window, removes the grid UUID from the `grids` registry in LSD and cleans up the namespace keys (`<grid_uuid>:macro:*`, `<grid_uuid>:micro:*`, `<grid_uuid>:targets`). Proc1 and Proc2 pick up the removal on their next round-robin pass and stop processing that namespace. Proc3 picks up the removal on its next cycle and stops configuring wind for that grid.
...
- **`grids`** — a CSV of all registered grid UUIDs. Main writes this; Proc1, Proc2, and Proc3 read it. Proc1/Proc2 use it for round-robin workload; Proc3 uses it to configure its wind driver per grid.
...
- **`<grid-uuid>:targets`** — per-grid current target parameters. Main writes these on grid registration and TARGET_PUSH; Proc2 reads them for evolution and target interpolation; Proc3 reads them to configure wind base values and detect state modifier changes.
- **`drivers:pressure`** — global atmospheric pressure driver owned by Proc3. Incorporates thermal and moisture coupling from `drivers:conditions` data. Proc1 and Proc2 read this when processing each grid; Proc3 writes it on its own cycle cadence.
- **`drivers:wind`** — global wind speed/direction driver owned by Proc3 (calm/gust oscillation, diurnal sea/land breeze, pressure-gradient wind). Proc2 reads this instead of relaxing toward notecard wind targets; Proc3 writes it on its own cycle cadence.
- **`drivers:conditions`** — global conditions data (temp, humidity) owned by Proc2. Proc3 reads this for pressure-wind coupling; Proc2 writes it on its own cycle cadence.
...
- Main signals procs to reinitialize from scratch. Proc3 reinitializes flood state, pressure driver, and wind driver. Proc1/Proc2 start with empty round-robin.
...
[Proc3] Drivers:
  ...
  wind: speed=8.2, dir=315, gust=12.5, variability=0.3
  conditions: temp=24.1, humidity=58
```
